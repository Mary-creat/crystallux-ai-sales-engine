...
- [x] **Webhook health probe + auto-alert** — `wfSentinelWebhookHealthProbeV1` runs every 30 min. Probes 11 critical endpoints (admin/list-leads, admin/comms-log, admin/system-health, admin/workflow-status, admin/sales-engine/activity, admin/chat, admin/market-intelligence/summary, admin/sentinel/comms/summary, admin/ciro/alerts, public/quote/fetch, public/mga/apply). Writes a sentinel_alerts row when any return 404, emails Mary immediately when 3+ are down (the "n8n needs restart" pattern). First self-healing gap-filler shipped.
