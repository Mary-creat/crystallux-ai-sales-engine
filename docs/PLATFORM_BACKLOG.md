...
- [x] **Postmark webhook ingestion** — `email_events` table + `clx-webhook-postmark-events-v1` receiver (validates X-Postmark-Webhook-Token, normalizes Delivery/Bounce/SpamComplaint/Open/Click/SubscriptionChange RecordTypes, ack 200). Comms tab now reads live spam + bounces from email_events when present, falls back to email_log scan otherwise. New Bounces card with subtype breakdown.
