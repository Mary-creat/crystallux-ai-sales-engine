...
| **1.11 Queue tier actually queues** | Platform | `PARTIAL` | `docker-compose.yml` ran main at `EXECUTIONS_MODE=regular` while `n8n-worker` ran `queue`. n8n only enqueues to Redis when the **main** instance is in queue mode, so the worker never received a job — a scaling tier declared, paid for in RAM, never reached. Main is now `queue` | none | **Repo only.** Whether the VPS runs this exact file is UNVERIFIED | applying restarts n8n | **YES** | [Owner action #5](OWNER_ACTIONS_REQUIRED.md) |
