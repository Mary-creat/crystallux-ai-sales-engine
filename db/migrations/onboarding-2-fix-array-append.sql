-- ============================================================
-- Onboarding fix — array append, not array concatenation
--
-- Found by running the proof rather than reasoning about it. The
-- function applied cleanly and then failed at call time with:
--
--   22P02 malformed array literal: "titles"
--   Array value must start with "{" or dimension information
--
-- Cause: `v_inherited := v_inherited || 'titles'` looks like appending an
-- element, and Postgres reads it as `anyarray || anyarray`. The untyped
-- literal is coerced to text[], 'titles' is not valid array syntax, and it
-- throws. Five occurrences, one per inheritable field.
--
-- array_append() says what was meant and cannot be read the other way.
--
-- CREATE OR REPLACE with the identical signature, so this replaces the
-- function in place. Nothing else changes; no data is touched.
-- ============================================================
...
-- ---- VERIFY ----
-- Expect ok=true, vertical=construction, and inherited_from_vertical
-- listing the five fields taken from niche_overlays.
SELECT upsert_client_icp_from_onboarding(
  '6edc687d-07b0-4478-bb4b-820dc4eebf5d'::uuid, 'construction') AS onboarding_proof;

-- Expect exactly 1 profile, for the test tenant, vertical construction.
SELECT client_id, vertical, geography, array_length(titles,1) AS title_count, is_active
  FROM client_icp_profiles;

-- Expect 8 rows, all untouched: onboarding must never write to the global
