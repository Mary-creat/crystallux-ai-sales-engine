...
(function (global) {
  ...
  /**
   * Render a list of stat cards. Items: { label, value, delta?, deltaDir?,
   * icon?, variant?, href? }. icon → inline-SVG; variant → color class;
   * href → clickable card. Backward-compatible.
   */
  ...
  function renderEmpty(el, message, iconName, ctaHtml) {
    ...
    if (iconName && ICONS[iconName]) {
      ...
      el.innerHTML = '<div class="clx-empty">' +
        escapeHtml(message || 'Nothing to show yet.') + '</div>';
    }
  }
  ...
  // ════════════════════════════════════════════════════════════════════
  // Polish helpers — mirror of admin-dashboard. Same API; client pages
  // can use the same icon / chart / skeleton / avatar helpers as admin.
  // ════════════════════════════════════════════════════════════════════
  ...
  function icon(name, size) {
    var path = ICONS[name];
    if (!path) return '';
    var sizeCls = size === 'sm' ? ' --sm' : size === 'lg' ? ' --lg' : size === 'xl' ? ' --xl' : '';
  ...
  function sparkline(values, options) {
    ...
    var firstX = pad.toFixed(1), lastX = (pad + (n - 1) * stepX).toFixed(1), bottom = (h - pad).toFixed(1);
  ...
  function donut(slices, options) {
    ...
    var arcs = slices.map(function (s, i) {
      ...
      var arc = '<circle cx="50" cy="50" r="' + radius + '" fill="none" stroke="' + color + '" stroke-width="14"' +
  ...
  /* ───────────────────────────────────────────────────────────────
     ...
     Mirrors admin-dashboard/shared/components.js. Pure vanilla.
     Styles in client layout.css under matching .clx-* classes.
     ─────────────────────────────────────────────────────────────── */
  ...
  function toast(message, opts) {
    ...
    var timer = null;
    if (duration > 0) timer = setTimeout(function () { dismiss(); }, duration);
    function dismiss() {
  ...
  function dialog(opts) {
    ...
    if (opts.actions && opts.actions.length) html += '<div class="clx-dialog-actions"></div>';
    box.innerHTML = html;
